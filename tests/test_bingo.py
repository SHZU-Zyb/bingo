...
from bingo.task_state import TaskState
...
def test_agent_caches_file_evidence_without_promoting_code_to_memory(tmp_path):
    ...
    assert not any("deploy key is red" in note["text"] for note in notes)
    assert agent.session["memory"]["file_summaries"] == {}
    assert agent.session["evidence_cache"]["entries"][0]["path"] == "facts.txt"
    ...
    memory_section = prompt.split("Working state:\n", 1)[1].split("\n\nTranscript:", 1)[0]
    assert "deploy key is red" not in memory_section
...
def test_file_summary_cache_is_invalidated_on_out_of_band_edit_and_path_spelling(tmp_path):
    ...
    assert "sample.txt" in agent.memory.render_memory_text()
...
def test_write_invalidates_cached_source_evidence(tmp_path):
    file_path = tmp_path / "sample.txt"
    file_path.write_text("alpha\n", encoding="utf-8")
    agent = build_agent(tmp_path, [])
...
def test_checkpoint_keeps_evidence_references_without_copying_source(tmp_path):
    ...
    agent = build_agent(tmp_path, [], retrieval_engine=engine)
    hit = engine.search("alpha")["hits"][0]
    agent.run_tool(
        "read_symbol",
        {"symbol_id": hit["symbol_id"], "expected_hash": hit["content_hash"], "max_chars": 2000},
    )

    checkpoint = agent.create_checkpoint(
        TaskState.create(task_id="tatest-key-cache", user_request="inspect alpha"),
        "inspect alpha",
        trigger="test",
    )

    assert checkpoint["key_files"] == [{"path": "sample.py", "freshness": hit["content_hash"]}]
    assert checkpoint["evidence_refs"] == [
        {
            "path": "sample.py",
            "start_line": 1,
            "end_line": 2,
            "file_hash": hit["content_hash"],
            "symbol_id": hit["symbol_id"],
        }
    ]
    assert "def alpha" not in str(checkpoint)


def test_prompt_metadata_reports_external_evidence_invalidation(tmp_path):
    ...
    metadata = agent.prompt_metadata("inspect sample", "")

    assert metadata["stale_evidence_invalidations"] == 1
    assert metadata["stale_evidence_paths"] == ["sample.txt"]
    assert agent.session["evidence_cache"]["entries"] == []


def test_reset_clears_evidence_cache_with_session_state(tmp_path):
    path = tmp_path / "sample.txt"
    path.write_text("alpha\n", encoding="utf-8")
    agent = build_agent(tmp_path, [])
    agent.run_tool("read_file", {"path": "sample.txt", "start": 1, "end": 1})
    ...
    assert agent.session["evidence_cache"]["entries"] == []
    assert agent.evidence_cache.to_dict()["entries"] == []
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".bingo").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".bingo" not in result
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["timeout"] == 30
    assert captured["headers"]["Authorization"] == "Bearer test-key-test"
    assert captured["headers"]["Content-type"] == "application/json"
    assert captured["headers"]["Accept"] == "application/json"
    assert captured["headers"]["User-agent"] == "bingo/0.1"
...
def test_anthropic_compatible_client_posts_expected_messages_payload():
    ...
    assert captured["timeout"] == 30
    assert captured["headers"]["X-api-key"] == "test-key-test"
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        os.environ,
        {
            ...
            "OPENAI_API_KEY": "test-key-test",
            "OPENAI_MODEL": "env-model",
        },
        clear=False,
    ):
        with patch(
            "bingo.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("bingo.cli.OpenAICompatibleModelClient") as mock_openai:
    ...
    assert mock_openai.call_args.kwargs["api_key"] == "test-key-test"
...
def test_build_arg_parser_progress_defaults_off_and_can_be_enabled(tmp_path):
    parser = mini_pkg.build_arg_parser()

    default_args = parser.parse_args(["--cwd", str(tmp_path)])
    progress_args = parser.parse_args(["--cwd", str(tmp_path), "--progress"])

    assert default_args.progress is False
    assert progress_args.progress is True


def test_build_agent_wires_progress_renderer_for_new_and_resumed_sessions(tmp_path):
    parser = mini_pkg.build_arg_parser()
    default_args = parser.parse_args(["--cwd", str(tmp_path), "--provider", "ollama"])
    with patch("bingo.cli.OllamaModelClient", return_value=FakeModelClient([])):
        default_agent = mini_pkg.build_agent(default_args)
    assert default_agent.event_sink is None

    progress_args = parser.parse_args(
        ["--cwd", str(tmp_path), "--provider", "ollama", "--resume", "latest", "--progress"]
    )
    with patch("bingo.cli.OllamaModelClient", return_value=FakeModelClient([])):
        resumed_agent = mini_pkg.build_agent(progress_args)

    assert isinstance(resumed_agent.event_sink, ConsoleProgressRenderer)
    assert resumed_agent.event_sink.max_steps == progress_args.max_steps
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        os.environ,
        {
            "OPENAI_API_KEY": "test-key-openai-fallback",
        },
        clear=True,
    ):
        with patch(
            ...
            "bingo.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("bingo.cli.AnthropicCompatibleModelClient") as mock_anthropic:
    ...
    assert mock_anthropic.call_args.kwargs["api_key"] == "test-key-openai-fallback"
    assert agent.model_client is fake_client
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("bingo.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_deepseek_provider_and_env_configuration(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                ...
                "BINGO_DEEPSEEK_API_KEY=test-key-project-deepseek",
    ...
    with patch.dict(
        os.environ,
        {
            ...
            "DEEPSEEK_API_KEY": "test-key-legacy-deepseek",
    ...
    assert mock_anthropic.call_args.kwargs["api_key"] == "test-key-project-deepseek"
    assert agent.model_client is fake_client
...
def test_build_agent_uses_deepseek_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(os.environ, {"DEEPSEEK_API_KEY": "test-key-deepseek"}, clear=True):
        with patch("bingo.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            mini_pkg.build_agent(args)
...
def test_trace_and_report_redact_secret_env_values(tmp_path):
    secret = "test-key-test-secret-123"
    # Python 3.11+ needs the Windows shell location even in an isolated environment.
    shell_env = {name: os.environ[name] for name in ("SYSTEMROOT", "COMSPEC") if name in os.environ}
    with patch.dict(os.environ, {**shell_env, "OPENAI_API_KEY": secret}, clear=True):
        agent = build_agent(
            tmp_path,
            [
...
def test_prompt_budget_metadata_records_budget_decisions(tmp_path):
    ...
    relevant_section = agent.model_client.prompts[0].split("Recalled memory:\n", 1)[1].split("\n\nTranscript:", 1)[0]
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .bingo/.\n"
    ...
    index_path = tmp_path / ".bingo" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".bingo" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".bingo" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .bingo/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    agent = build_agent(
        ...
        [
            "<final>Project convention: Use constrained tools instead of guessing.\n"
            "Dependency: API key is test-key-live-secret-abc.\n"
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".bingo" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".bingo" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(mini_pkg.__file__).as_posix().endswith("/bingo/__init__.py")
