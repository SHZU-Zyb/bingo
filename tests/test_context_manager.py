...
def test_context_manager_assembles_sections_in_expected_order(tmp_path):
    ...
    assert prompt.index("You are bingo") < prompt.index("Working state:")
    assert prompt.index("Working state:") < prompt.index("Recalled memory:")
    assert prompt.index("Recalled memory:") < prompt.index("Transcript:")
...
def test_context_manager_does_not_repeat_current_request_in_working_state(tmp_path):
    agent = build_agent(tmp_path, [])
    agent.memory.set_task_summary("unique-current-request")

    prompt, _ = ContextManager(agent).build("unique-current-request")

    assert prompt.count("unique-current-request") == 1
...
def test_context_manager_renders_top_three_episodic_notes_per_note_under_budget(tmp_path):
    ...
    relevant_section = prompt.split("Recalled memory:\n", 1)[1].split("\n\nTranscript:", 1)[0]
...
def test_context_manager_separates_candidates_cached_source_and_recalled_memory(tmp_path):
    source = "def search_code(query):\n    return query.strip()\n"
    (tmp_path / "retrieval.py").write_text(source, encoding="utf-8")
    agent = build_agent(
        tmp_path,
        [],
        retrieval_engine=RetrievalEngine(tmp_path, small_chunks=0),
        auto_retrieve=True,
    )
    agent.memory.append_note(
        "Decision: keep symbol cards short",
        tags=("decision", "symbol"),
        source="key-decisions",
        kind="durable",
    )
    agent.evidence_cache = EvidenceCache(agent.session["evidence_cache"], workspace_root=tmp_path)
    hit = agent.get_retrieval_engine().search("search_code")["hits"][0]
    agent.evidence_cache.store(
        "retrieval.py",
        1,
        2,
        "# retrieval.py\n   1: def search_code(query):\n   2:     return query.strip()",
        symbol_id=hit["symbol_id"],
        file_hash=hit["content_hash"],
    )

    prompt, metadata = ContextManager(agent, total_budget=16000).build(
        "按照之前的 symbol 决定修改 search_code 方法"
    )

    assert "Recalled memory:" in prompt
    assert "Decision: keep symbol cards short" in prompt
...
def test_context_manager_drops_stale_cached_source_evidence(tmp_path):
    (tmp_path / "sample.py").write_text("def alpha():\n    return 1\n", encoding="utf-8")
    engine = RetrievalEngine(tmp_path)
    agent = build_agent(tmp_path, [], retrieval_engine=engine, auto_retrieve=True)
    hit = engine.search("alpha")["hits"][0]
    agent.evidence_cache.store(
        "sample.py", 1, 2, "def alpha():\n    return 1", symbol_id=hit["symbol_id"]
    )
    (tmp_path / "sample.py").write_text("def alpha():\n    return 2\n", encoding="utf-8")

    prompt, metadata = ContextManager(agent).build("sample.py 的 alpha 函数")

    assert "Cached source evidence" not in prompt
    assert metadata["source_evidence"]["cache_hits"] == 0
    assert metadata["source_evidence"]["stale_invalidations"] == 1


def test_context_manager_dedupes_history_read_covered_by_source_evidence(tmp_path):
    source = "def search_code(query):\n    return query.strip()\n"
    (tmp_path / "retrieval.py").write_text(source, encoding="utf-8")
    engine = RetrievalEngine(tmp_path, small_chunks=0)
    agent = build_agent(tmp_path, [], retrieval_engine=engine, auto_retrieve=True)
    hit = engine.search("search_code")["hits"][0]
    content = "# retrieval.py\n   1: def search_code(query):\n   2:     return query.strip()"
    agent.evidence_cache.store(
        "retrieval.py", 1, 2, content, symbol_id=hit["symbol_id"], file_hash=hit["content_hash"]
    )
    agent.record(
        {
            "role": "tool",
            "name": "read_symbol",
            "args": {"symbol_id": hit["symbol_id"]},
            "content": content,
            "created_at": "2026-09-08T10:00:00+00:00",
        }
    )

    prompt, metadata = ContextManager(agent).build("修改 search_code 方法")

    assert prompt.count("def search_code(query):") == 1
    assert metadata["history"]["deduped_source_evidence_count"] == 1


def test_direct_retrieval_does_not_repeat_the_same_cached_source(tmp_path):
    source = "def alpha():\n    return 1\n"
    (tmp_path / "sample.py").write_text(source, encoding="utf-8")
    engine = RetrievalEngine(tmp_path)
    agent = build_agent(tmp_path, [], retrieval_engine=engine, auto_retrieve=True)
    agent.evidence_cache.store("sample.py", 1, 2, source)

    prompt, metadata = ContextManager(agent).build("检查 sample.py 文件")

    assert metadata["retrieval"]["strategy"] == "direct"
    assert prompt.count("def alpha():") == 1
    assert metadata["source_evidence"]["cache_hits"] == 0


def test_disabled_context_reduction_still_omits_empty_recalled_memory(tmp_path):
    ...
    assert metadata["context_route"]["use_memory_recall"] is False


def test_history_compression_never_reuses_a_stale_evidence_summary(tmp_path):
    ...
    agent.record(
        {
            "role": "tool",
            "name": "read_file",
            "args": {"path": "sample.txt", "start": 1, "end": 1},
            "content": result,
        ...
        agent.record(
            {
                ...
                "created_at": f"2026-09-08T09:0{index + 1}:00+00:00",
            }
        )
    path.write_text("NEW_VALUE\n", encoding="utf-8")

    prompt, _ = ContextManager(agent).build("continue")
