...
session history 负责保存完整事件流；这个模块保存当前任务的轻量引用，
以及少量跨轮仍有价值的过程笔记和持久决策。旧 Session 的文件摘要字段
仍可读取和失效，但新源码观察由 EvidenceCache 管理，也不会在 Prompt 展开。
"""
...
def _tokenize(text):
    value = str(text)
    result = {token.lower() for token in re.findall(r"[A-Za-z0-9_]+", value)}
    # Chinese text has no whitespace boundaries.  Character bigrams preserve useful
    # phrases such as “架构决定” without pulling a tokenizer dependency into memory.
...
def _is_code_derived_note(note, workspace_root=None):
    kind = str(note.get("kind", "episodic")).strip() or "episodic"
    if kind in {"durable", "process"}:
        return False
    source = str(note.get("source", "")).strip()
    if not source:
        return False
    resolved = resolve_workspace_path(source, workspace_root)
    if resolved is not None and resolved.is_file():
        return True
    return bool(re.search(r"(?i)(^|[\\/])[^\\/]+\.(py|pyi|js|jsx|ts|tsx|java|go|rs|c|cc|cpp|h|hpp|cs|rb|php|swift|kt|kts|scala|sh|ps1|md|json|ya?ml|toml)$", source))
...
def normalize_memory_state(state, workspace_root=None):
    ...
    durable_root = Path(workspace_root) / ".bingo" / "memory" if workspace_root is not None else None
...
def retrieval_candidates(state, query, limit=3, workspace_root=None):
    ...
    for note in state["episodic_notes"]:
        # Reads from the repository belong to EvidenceCache/Code Retrieval.  Old
        # sessions can still contain them, so filter them at recall time.
        if _is_code_derived_note(note, workspace_root):
            continue
        # 召回逻辑故意保持简单透明：先看 tag 精确命中，
        # 再看关键词重叠，最后看新旧程度。这里不引入 embedding。
        note_tags = {tag.lower() for tag in note.get("tags", [])}
        tag_tokens = set().union(*(_tokenize(tag) for tag in note_tags)) if note_tags else set()
        note_tokens = _tokenize(note.get("text", "")) | _tokenize(note.get("source", "")) | note_tags | tag_tokens
        exact_tag_match = int(bool(query_tokens & note_tags))
        keyword_overlap = len(query_tokens & note_tokens)
        if exact_tag_match == 0 and keyword_overlap == 0:
            continue
    ...
    if workspace_root is not None:
        durable_store = DurableMemoryStore(Path(workspace_root) / ".bingo" / "memory")
    ...
    for _, note in ranked:
        identity = " ".join(str(note.get("text", "")).split()).casefold()
...
class LayeredMemory:
    def __init__(self, state=None, workspace_root=None):
        ...
        self.durable_store = DurableMemoryStore(Path(workspace_root) / ".bingo" / "memory") if workspace_root is not None else None
