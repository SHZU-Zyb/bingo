...
OPENAI_COMPATIBLE_USER_AGENT = "bingo/0.1"


class FakeModelClient:
    def __init__(self, outputs, _shared=None):
        self._shared = _shared or {
            "outputs": list(outputs),
            "prompts": [],
            "lock": threading.Lock(),
        }
        self.outputs = self._shared["outputs"]
        self.prompts = self._shared["prompts"]
    ...
    def clone(self):
        return FakeModelClient([], _shared=self._shared)
...
class OllamaModelClient:
    ...
    def clone(self):
        return OllamaModelClient(self.model, self.host, self.temperature, self.top_p, self.timeout)
...
class OpenAICompatibleModelClient:
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        在 agent 链路里的位置：
        它位于 `Bingo.ask()` 的模型调用阶段，是稳定前缀缓存复用链路真正
        落到 provider API 的地方。
        """
        self.last_completion_metadata = {}
        payload = {
...
def clone_model_client(client):
    """Return a state-isolated client for one child Agent."""
    clone = getattr(client, "clone", None)
    if not callable(clone):
        raise TypeError("model client does not support isolated child Agents")
    result = clone()
    if result is client:
        raise RuntimeError("model client clone must return a distinct instance")
    return result
